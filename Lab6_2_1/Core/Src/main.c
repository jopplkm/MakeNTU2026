...
void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef *hadc) {
    osThreadFlagsSet(defaultTaskHandle, FLAG_HALF_CPLT);
}

// DMA 全滿時觸發
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
    osThreadFlagsSet(defaultTaskHandle, FLAG_FULL_CPLT);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    // 2. 手動初始化 DMA (取代隱藏檔案裡可能遺失的設定)
    hdma_adc1.Instance = DMA1_Channel1;
    hdma_adc1.Init.Request = DMA_REQUEST_0; // STM32L4 的 ADC1 對應 Request 0
    ...
    // 3. 強制將 DMA 與 ADC 綁定！ (這是最關鍵的一步)
    __HAL_LINKDMA(&hadc1, DMA_Handle, hdma_adc1);

    // 4. ADC 啟動前校正
    HAL_ADCEx_Calibration_Start(&hadc1, ADC_SINGLE_ENDED);

    // 5. 啟動 ADC DMA，並加上成功/失敗的文字檢查
    if (HAL_ADC_Start_DMA(&hadc1, (uint32_t *)sample_buffer, SAMPLE_BUFFER_SIZE) != HAL_OK) {
        char msg[] = "ADC DMA Start FAILED! \r\n";
        HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
    } else {
        char msg[] = "ADC DMA Started Successfully!\r\n";
        HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
    }
...
void StartDefaultTask(void *argument)
{
  /* USER CODE BEGIN 5 */
	  ...
	  // 開機存活證明
	  sprintf(msg, "=== System Started! Waiting for ADC... ===\r\n");
	  HAL_UART_Transmit(&huart1, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
