...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    {
      // 讀取硬體自動抓好的數值
      capture_period = HAL_TIM_ReadCapturedValue(&htim3, TIM_CHANNEL_1);
      capture_high = HAL_TIM_ReadCapturedValue(&htim3, TIM_CHANNEL_2);

      // 避免剛開機時分母為 0 導致程式當掉
      if (capture_period != 0) {
          // 計算佔空比 (%)：高電位時間 / 總週期時間
          measured_duty = ((float)capture_high / (float)capture_period) * 100.0f;

          // 計算頻率 (Hz)：TIM3 經過除頻後的速度是 10,000 Hz
          measured_freq = 10000.0f / (float)capture_period;
      }

      HAL_Delay(200); // 稍微延遲一下，方便觀察數值
    /* USER CODE END WHILE */
