...
#define APP_ADC_TX_PERIOD_MS 20U
/** Hit detector thresholds with hysteresis (pulse units 0..4095). */
#define APP_EMG_HIT_THRESHOLD_HIGH 400U
#define APP_EMG_HIT_THRESHOLD_LOW 250U
/** Minimum gap between hit pulses to avoid multi-trigger on one contraction. */
#define APP_EMG_HIT_COOLDOWN_MS 80U
/** Require some rising edge to avoid constant-noise retrigger. */
#define APP_EMG_HIT_MIN_RISE 8U
/** Press USER button then keep relaxed for this calibration window. */
#define APP_EMG_CALIBRATION_MS 10000U
/** Runtime thresholds computed from relaxed baseline average. */
#define APP_EMG_LOW_OFFSET 100U
#define APP_EMG_HIGH_OFFSET 200U
/** Merge brief dips into same effort event (high->low->high). */
#define APP_EMG_EVENT_END_GAP_MS 100U

/**
 * PC → MCU on the same WiFi TCP socket: text lines "jdg:perfect\\r\\n", "jdg:good\\r\\n", "jdg:miss\\r\\n".
 * WS2812 on ARD_D5 (PB4): APP_WS2812_USE_PWM_DMA=1 uses TIM3_CH1 + TIM3_UP DMA (recommended).
 * =0 uses GPIO bit-bang. ARD_D4 is TIM2 — do not wire WS2812 DIN to D4.
 */
...
/** 1 = print PC→MCU TCP text lines on UART1 (ST-Link VCP) for debugging */
#define APP_PC_RX_LOG_UART 1

extern ADC_HandleTypeDef hadc1;
extern DMA_HandleTypeDef hdma_adc1;

/** Set 0 if you still need Cube MX_SPI3_Init (e.g. other SPI slaves on same bus). EsWifi replaces SPI3 init when 1. */
#define APP_USE_WIFI 1

#if APP_USE_WIFI
#define APP_WIFI_SSID "Zenfone10"
#define APP_WIFI_PASSWORD "141421356237"
/** Server / PC running TCP listener (e.g. Python socket), same LAN as STA. */
#define APP_WIFI_REMOTE_IP0 10U
#define APP_WIFI_REMOTE_IP1 238U
#define APP_WIFI_REMOTE_IP2 173U
